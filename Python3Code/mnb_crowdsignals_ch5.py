...
def main():
    ...
    if FLAGS.mode == 'final':
        ...
        dataset = clusteringNH.k_means_over_instances(dataset, ['mag_phone_x', 'mag_phone_y', 'mag_phone_z'], FLAGS.k, 'default', 50, 50)
        DataViz.plot_clusters_3d(dataset, ['mag_phone_x', 'mag_phone_y', 'mag_phone_z'], 'cluster', ['label'])
        DataViz.plot_silhouette(dataset, 'cluster', 'silhouette')
        util.print_latex_statistics_clusters(
            dataset, 'cluster', ['mag_phone_x', 'mag_phone_y', 'mag_phone_z'], 'label')
